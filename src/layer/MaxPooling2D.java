...
public class MaxPooling2D extends Layer {
    ...
    public MaxPooling2D(int[] shape, int[] poolSize, int[] strides) {
        ...
        out_H = (1 + (inputH - poolHeight) / stride1);
        out_W = (1 + (inputW - poolWidth) / stride2);
    ...
    public MaxPooling2D(int[] shape, int poolSize) {
        ...
        out_H = (1 + (inputH - poolHeight) / stride1);
        out_W = (1 + (inputW - poolWidth) / stride1);
    ...
    public int getOutputRows() {
        int h_out = (1 + (this.inputH - poolHeight) / stride1);
        return h_out;
    ...
    public int getOutputCols() {
        return (1 + (this.inputW - poolWidth) / stride1);
    ...
    public double[][][][] forward(double[][][][] input) {
        ...
        int h_out = (1 + (H - poolHeight) / stride1);
        int w_out = (1 + (W - poolWidth) / stride1);
            ...
            for (int i = 0; i < C; i++) {
                ...
                for (int hi = 0; hi < h_out; hi++) {
                    for (int wi = 0; wi < w_out; wi++) {
                        tmp = getSubmatrixMaxAndIndex(input[bs][i], hi * stride1, hi * stride1 + poolHeight, wi * stride1, wi * stride1 + poolWidth);
    ...
    public double[][][] forward(double[][][] input) {
        ...
        for (int i = 0; i < C; i++) {
            ...
            for (int hi = 0; hi < h_out; hi++) {
                for (int wi = 0; wi < w_out; wi++) {
                    tmp = getSubmatrixMaxAndIndex(input[i], hi * stride1, hi * stride1 + poolHeight, wi * stride1, wi * stride1 + poolWidth);
                    out[i][hi][wi] = tmp[0];
                    hb = (int) tmp[1];
                    wb = (int) tmp[2];
                    tmpBack[hb][wb] = tmp[0];
    ...
    public String export() {
        return "maxpooling2d;" + poolHeight + ";" + poolWidth + ";" + stride1 + ";" + stride2 + ";" + writeShape(getInputShape());
    }
