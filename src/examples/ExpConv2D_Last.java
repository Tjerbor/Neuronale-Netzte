...
public class ExpConv2D_Last {
    ...
    public static void main(String[] args) throws IOException {
        ...
        builder.addMaxPooling2D_Last(2, 2); //uses for standard strides2 and poolSize2
        ...
        for (int i = 0; i < x_train.length; i++) {
            x_convT[i] = reshapeToChannelsLast(x_train[i], new int[]{28, 28, 1});
        }

        for (int i = 0; i < x_test.length; i++) {
            x_convTest[i] = reshapeToChannelsLast(x_test[i], new int[]{28, 28, 1});
        }
        ...
        nn.writeModel("nn_weights_conv_last.txt");
        nn = LoadModel.loadModel("nn_weights_conv_last.txt");
        nn.printSummary();
